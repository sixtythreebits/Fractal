...
function InitCollegeAutocompleteTextbox(TextBoxSelector) {
    $(TextBoxSelector).autocomplete({
        delay: 500,
        minLengthType: 2,
        source: function (req, resp) {
            if (req.term) {
                $.ajax({
                    type: "POST",                    
                    url: Api + "Utility/SearchCollegeByCaption/json/",
                    dataType: 'json',                    
...
function InitTracking() {
    ...
    if (val != undefined && val.length > 0) {
        setInterval(function () {
            $.ajax({
                url: Api + "track/UpdateTrackTime/",
                type: "POST",
                data: { id: val, time: (TimeSpentOnPage += 5) }
            });
        }, 5000);
    }
}
